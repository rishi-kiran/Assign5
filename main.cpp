...
int main(int argc, char *argv[]) {
    ...
    cout << "The duration is: " << fixed << time_taken << setprecision(5) << " seconds" << endl;
    ...
    for (std::vector<int>::const_iterator it = large_refs.begin(); it != large_refs.end(); ++it) {
        int page_num = (*it) >> page_offset_bits;
        bool isPageFaultss = lf.access_page(page_num, 0);
    }
    ...
    LRUReplacement x(num_pages, num_frames);
    clock_gettime(CLOCK_MONOTONIC, &start);
    for (std::vector<int>::const_iterator it = large_refs.begin(); it != large_refs.end(); ++it) {
        int page_num = (*it) >> page_offset_bits;
        bool isPageFault = x.access_page(page_num, 0);
    } 
